...
// MyStrCPY(WPreturnbuff+17, WP_UserBalance, CARD_SIZE);        //拷贝卡上余额
 ...
 MyStrCPY(WPreturnbuff+21, Consum_Amount, CARD_SIZE);      //拷贝消费金额		
...
/*----------------------------------------------------------------------------*/	
	...
	{ 
		...
		else if (WP_u8RxIndexW  > (WPData_length + 5))  {goto com_end;}
	}
 else 
	{
			  ...
			  case 0x02:    
				{
						...
						MyStrCPY(idbuff, WP_RxDataBuff+13, 4);           //扣款时的卡号	
											
						ProcessCard(WP_readcard_ret);            break;				  //消费
