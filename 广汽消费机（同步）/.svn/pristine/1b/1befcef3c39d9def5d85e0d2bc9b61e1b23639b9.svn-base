...
unsigned char S2_Mode =3;	//端口1的运行模式,0:TCP服务器模式,1:TCP客户端模式,2:UDP(广播)模式
...
unsigned char S2_Data;		  //端口1接收和发送数据的状态,1:端口接收到数据,2:端口发送数据完成 
...
/***************----- 端口2数据缓冲区 -----***************/
unsigned char S2_Rx_Buffer[100];	//端口接收数据缓冲区 
unsigned char S2_Tx_Buffer[100];	//端口发送数据缓冲区 
...
void W5500_Hardware_Reset(void)
{
	...
	Delay(200);
//	while((Read_W5500_1Byte(PHYCFGR)&LINK)==0);//等待以太网连接完成 ...201707
}
...
void Socket_Init(SOCKET s)
{
	...
	Write_W5500_SOCK_2Byte(2, Sn_MSSR, 100);//最大分片字节数=1460(0x5b4)
	...
	switch(s)
	{
		...
		case 2:
						//设置端口1的端口号
      Write_W5500_SOCK_2Byte(2, Sn_PORT, UDP_DPORT[0]*256+UDP_DPORT[1]);	
			break;
