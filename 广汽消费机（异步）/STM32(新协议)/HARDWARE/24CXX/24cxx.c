...
/*---------------------------------页写入-------------------------------------*/
 void  AT24CXX_Write(u16 address, u8 *date , u16 number)
{
	u8 i;
	IIC_Start();
	...
	IIC_Send_Byte((address & 0xFFFF) >> 8);			//内存地址高8位
	
	IIC_Wait_Ack();  
	
	IIC_Send_Byte(address & 0xFF);      	//内存地址低8位
	
	IIC_Wait_Ack(); 
	
	for(i=0;i<number;i++)	    
	{	
		IIC_Send_Byte(*date++);   		//写入数据	
