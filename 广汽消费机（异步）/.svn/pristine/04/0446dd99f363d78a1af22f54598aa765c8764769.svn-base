...
//	uint8 buff[512];                                                          /*定义一个2KB的缓存*/	
	switch(getSn_SR(SOCK_UDPS))                                                /*获取socket的状态*/
	{
		...
		case SOCK_UDP:                                                           /*socket初始化完成*/
			...
			{
				recvfrom(SOCK_UDPS,UDPClient_databuff, len, remote_ip,&remote_port);               /*W5500接收计算机发送来的数据*/
				UDPClient_databuff[len-8]=0x00;                                                    /*添加字符串结束符*/
		//		memcpy(UDPClient_databuff, buff,20);
			//	printf("%s\r\n",buff);                                               /*打印接收缓存*/ 
