...
#define   macLCD_DELAY()  delay_us (5)

extern volatile uint Bcolor;
extern volatile uint Fcolor;
extern u32 Layer0,Layer1,Layer2,Layer3;
extern  u8 DRAM_FREQ ,CORE_FREQ,SCAN_FREQ,E0VALUE,MPU_Interface,FLASHTYPE,TOUCHTYPE,FONT;
...
void LCD43inch_Init(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
	/*!< Configure SPI_LCD_SPI pins: CS */
	macLCD_GPIO_CS_CLK_FUN ( macLCD_GPIO_CS_CLK, ENABLE );
  GPIO_InitStructure.GPIO_Pin = macLCD_GPIO_CS_PIN;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = macLCD_GPIO_CS_Mode;
  GPIO_Init(macLCD_GPIO_CS_PORT, &GPIO_InitStructure);
	
  /*!< Configure SPI_LCD_SPI pins: SCK */
	macLCD_GPIO_SCK_CLK_FUN ( macLCD_GPIO_SCK_CLK, ENABLE );
  GPIO_InitStructure.GPIO_Pin = macLCD_GPIO_SCK_PIN;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = macLCD_GPIO_SCK_Mode;
  GPIO_Init(macLCD_GPIO_SCK_PORT, &GPIO_InitStructure);
	
  /*!< Configure SPI_LCD_SPI pins: MOSI */
	macLCD_GPIO_MOSI_CLK_FUN ( macLCD_GPIO_MOSI_CLK, ENABLE );
  GPIO_InitStructure.GPIO_Pin = macLCD_GPIO_MOSI_PIN;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = macLCD_GPIO_MOSI_Mode;
  GPIO_Init(macLCD_GPIO_MOSI_PORT, &GPIO_InitStructure);

  /*!< Configure SPI_LCD_SPI pins: MISO */
	macLCD_GPIO_MISO_CLK_FUN ( macLCD_GPIO_MISO_CLK, ENABLE );
  GPIO_InitStructure.GPIO_Pin = macLCD_GPIO_MISO_PIN;
  GPIO_InitStructure.GPIO_Mode = macLCD_GPIO_MISO_Mode;
  GPIO_Init(macLCD_GPIO_MISO_PORT, &GPIO_InitStructure);	
	
	
  /*!< Configure SPI_LCD_SPI pins: RST */
	macLCD_GPIO_RST_CLK_FUN ( macLCD_GPIO_RST_CLK, ENABLE );
  GPIO_InitStructure.GPIO_Pin = macLCD_GPIO_RST_PIN;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = macLCD_GPIO_RST_Mode;
  GPIO_Init(macLCD_GPIO_RST_PORT, &GPIO_InitStructure);
}


///////////////////////以下为基本读写函数，请客户根据不同单片机，更改以上函数///////////////////


/*
 * 函数名：SPI_LCD43_SendByte
 * 描述  ：向LCD发送1 Byte 数据
 * 输入  ：byte，要发送的数据
 * 返回  : LCD返回的数据
 * 调用  ：内部调用
 */
void SPI_LCD43_SendByte ( u8 byte )
{
...
/*
 * 函数名：SPI_LCD43_ReadByte
 * 描述  ：从LCD发送1 Byte 数据
 * 输入  ：无
 * 返回  : LCD返回的数据
 * 调用  ：内部调用
 */
unsigned char SPI_LCD43_ReadByte ( void )
{
	unsigned char counter;
...
void LCD_CmdWrite(uchar _ucRegAddr)
{
	...
	macLCD_CS_0();
	SPI_LCD43_SendByte(SPI_WRITE_CMD);
	SPI_LCD43_SendByte(_ucRegAddr);
	macLCD_CS_1();
 ...
 void LCD_DataWrite(uint _ucRegValue)
{
	...
	macLCD_CS_0();
	SPI_LCD43_SendByte(SPI_WRITE_DATA);
	SPI_LCD43_SendByte(_ucRegValue);
	macLCD_CS_1();
}
 ...
 uint LCD_DataRead(void)
{
 ...
 uint value=0;
	...
	macLCD_CS_0();
	SPI_LCD43_SendByte(SPI_READ_DATA);
	value = SPI_LCD43_ReadByte();
	macLCD_CS_1();
 ...
 uchar LCD_StatusRead(void)
{
	...
	macLCD_CS_0();
	SPI_LCD43_SendByte(SPI_READ_STATUS);
	value = SPI_LCD43_ReadByte();
	macLCD_CS_1();	
...
void RA8871_InitHard43(void)
{
	
	RA8876_HW_Reset();             //显示屏复位

  RA8876_PLL_Initial();          //倍频
	
  RA8876_SDRAM_initail();
	
	TFT_24bit();
	
	if(MPU_Interface==2) //如果用到FSMC 16位的话，要设置成16位接口，其余都是8bit接口
	{  Host_Bus_16bit();
	}
	else
	Host_Bus_8bit();
...
RA8876_HW_Reset(); 
  ...
  RA8876_SDRAM_initail();	
	...
	void RA8876_InitHard70101(void)
{
RA8876_HW_Reset();
	  RA8876_PLL_Initial();
  RA8876_SDRAM_initail();
...
void screeninit(void)
{
  Select_Main_Window_16bpp();
	...
	Main_Image_Start_Address(Layer1);				
...
void LCD_Select(uchar lcdnum)
{
	...
	lcdx=480;   lcdy=272;
	Layer0 =0;	Layer1=786432	;Layer2=1835008	;	Layer3=2883584;	
	DRAM_FREQ = 100,CORE_FREQ   = 100,SCAN_FREQ = 30;E0VALUE=0x08;
	 
	RA8871_InitHard43();
	
	screeninit();
   
//if(MPU_Interface==0) //如果是用spi接口，初始化完成后，可以将spi速度调高，最高支持18M
//{
//HighSpeedSPI2();
//}
...
void RA8876_SW_Reset(void)//IC Resetぃ穦睲Register/Memory
{
	unsigned char temp;
...
void FLASH_Read_1024GMb(unsigned char *UpdataValue,unsigned short ReadAddr)//从液晶内部flash读取图片位置信息（针对1024Mb容量）
{
